<template>
  <div class="app-container">
    ...
    <el-dialog :title="$t('ChooseAttentionBuyer')" :visible.sync="selectBuyer" width="500px" center :close-on-click-modal="false">
      <el-select v-model="selectBuyerId" clearable style="width: 100%" class="filter-item" v-loading="buyerListLoading">
    ...
    <!--// 选择卖家经纪人弹窗-->
    <el-dialog :title="$t('table.changeBroker')" :visible.sync="selectBusinessBroker" width="500px" center :close-on-click-modal="false">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="changeBrokerSave(business_id,owner_id)">{{ $t('table.changeBroker') }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  import { getBusinessList,addBusiness,editBusiness,delBusiness,showBusiness,changeStatus,getBuyers,setAttentionBusiness,getBusinessbrokersList,changeBusinessbrokerSave } from '@/api/business'
    export default {
      ...
      methods:{
        ...
        changeBroker(row){
          let that=this;
          if(row){this.business_id=row.row.id;}
