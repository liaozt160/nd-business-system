...
<template>
  <div class="app-container">
    <aside>
      <h3 style="text-align: center;">{{$t('route.MoreBusinessInformation')}}</h3>
    </aside>
    ...
    <el-dialog :title="$t('detail')" :visible.sync="dialogView" width="800px" center>
      <el-form label-position="left" inline  class="demo-table-expand"  v-loading="ViewLoading">
        ...
        <el-form-item :label="$t('employeeEdit.GrossIncome')+'：'">
          <!--毛利润-->
          <span>{{ attentionData.gross_income }}</span>
        </el-form-item>
        ...
        <el-form-item :label="$t('employeeEdit.NetIncome')+'：'">
          <!--净利润-->
          <span>{{ attentionData.net_income }}</span>
        </el-form-item>
        <el-form-item :label="$t('employeeEdit.Lease')+'：'">
          <!--租金-->
          <span>{{ attentionData.lease }}</span>
        </el-form-item>
        ...
        <el-form-item :label="$t('employeeEdit.USBroker')+'：'">
          <!--美国中介-->
          <span>{{ attentionData.us_broker }}</span>
        </el-form-item>
        <el-form-item :label="$t('employeeEdit.USBrokerContactInfo')+'：'">
          <!--美国中介联系方式-->
          <span>{{ attentionData.us_broker_contact_info }}</span>
        </el-form-item>
        <el-form-item :label="$t('employeeEdit.ListingDate')+'：'">
          <!--待售开始日期-->
          <span>{{ attentionData.listing_date }}</span>
        </el-form-item>
        <el-form-item :label="$t('employeeEdit.ReasonForSelling')+'：'">
          <!--出售原因-->
          <span>{{ attentionData.reason_for_selling }}</span>
        </el-form-item>
      </el-form>
...
<script>
  ...
  import { buyerGetBusinessTwoList,showBusiness,showLevelTwo,getBuyers,setAttentionBusiness } from '@/api/business'
  export default {
    ...
    methods:{
      ...
      getList(data) {
        ...
        buyerGetBusinessTwoList (data).then(response => {
          console.log('businessForSalesLevelTwo',response);
